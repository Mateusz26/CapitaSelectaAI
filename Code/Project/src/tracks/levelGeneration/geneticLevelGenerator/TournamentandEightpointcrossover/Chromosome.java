...
public class Chromosome implements Comparable<Chromosome>{
	...
	public ArrayList<Chromosome> uniformCrossOver(Chromosome c){
		ArrayList<Chromosome> children = new ArrayList<Chromosome>();
		children.add(new Chromosome(level[0].length, level.length));
		children.add(new Chromosome(level[0].length, level.length));
	...
	public ArrayList<Chromosome> kPointCrossOver(Chromosome c, int k){
		ArrayList<Chromosome> children = new ArrayList<Chromosome>();
		children.add(new Chromosome(level[0].length, level.length));
		children.add(new Chromosome(level[0].length, level.length));
		Random rand = new Random();
		boolean schakel = false;
		ArrayList<Position> positions = new ArrayList<>(k);
		for(int i = 0; i < k; i++) {
			positions.add(new Position(rand.nextInt(level[0].length),rand.nextInt(level.length)));
		}
	...
	public void setLevel(ArrayList<String> s) {
		for(int y = 0; y < level.length; y++) {
			for (int x = 0; x < level[y].length; x++) {
				this.level[y][x].addAll(s);
			}
		}
	}
